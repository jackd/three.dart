...
init() {
  ...
  camera = new THREE.PerspectiveCamera(
      75.0, window.innerWidth / window.innerHeight, 1.0, 5000.0);
  ...
  for (int i = 0; i < table.length; i++) {
    ...
    var details = new Element.tag('div')
      ..classes.add('details')
      ..innerHtml = '${item.name}<br>${item.mass}';

    var element = new Element.tag('div')
      ..classes.add('element')
      ..style.backgroundColor =
          'rgba(0,127,127,${( new Math.Random().nextDouble() * 0.5 + 0.25 ) })'
      ..children.add(number)
      ..children.add(symbol)
  ...
  for (int i = 0; i < objects.length; i++) {
    ...
    targets["table"].add(new THREE.Object3D()
      ..position.x = (item.n0 * 160.0) - 1540.0
      ..position.y = -(item.n1 * 200.0) + 1100.0);
  }
  ...
  document
      .querySelector('#table')
      .onClick
      .listen((e) => transform(e.target, targets["table"], 2000));

  document
      .querySelector('#sphere')
      .onClick
      .listen((e) => transform(e.target, targets["sphere"], 2000));

  document
      .querySelector('#helix')
      .onClick
      .listen((e) => transform(e.target, targets["helix"], 2000));

  document
      .querySelector('#grid')
      .onClick
      .listen((e) => transform(e.target, targets["grid"], 2000));
