...
class WebGLRenderer implements Renderer {
  ...
  WebGLRenderer(
    ...
    supportsBoneTextures =
        supportsVertexTextures && (_glExtensionTextureFloat != null);

    // var _compressedTextureFormats = (_glExtensionCompressedTextureS3TC != null)
    //     ? _gl.getParameter(gl.COMPRESSED_TEXTURE_FORMATS)
    //     : [];

    //

    var _vertexShaderPrecisionHighpFloat =
        _gl.getShaderPrecisionFormat(gl.VERTEX_SHADER, gl.HIGH_FLOAT);
    var _vertexShaderPrecisionMediumpFloat =
        _gl.getShaderPrecisionFormat(gl.VERTEX_SHADER, gl.MEDIUM_FLOAT);
    // var _vertexShaderPrecisionLowpFloat =
    //     _gl.getShaderPrecisionFormat(gl.VERTEX_SHADER, gl.LOW_FLOAT);

    var _fragmentShaderPrecisionHighpFloat =
    ...
    // var _fragmentShaderPrecisionLowpFloat =
    //     _gl.getShaderPrecisionFormat(gl.FRAGMENT_SHADER, gl.LOW_FLOAT);
    //
    // var _vertexShaderPrecisionHighpInt =
    //     _gl.getShaderPrecisionFormat(gl.VERTEX_SHADER, gl.HIGH_INT);
    // var _vertexShaderPrecisionMediumpInt =
    //     _gl.getShaderPrecisionFormat(gl.VERTEX_SHADER, gl.MEDIUM_INT);
    // var _vertexShaderPrecisionLowpInt =
    //     _gl.getShaderPrecisionFormat(gl.VERTEX_SHADER, gl.LOW_INT);
    //
    // var _fragmentShaderPrecisionHighpInt =
    //     _gl.getShaderPrecisionFormat(gl.FRAGMENT_SHADER, gl.HIGH_INT);
    // var _fragmentShaderPrecisionMediumpInt =
    //     _gl.getShaderPrecisionFormat(gl.FRAGMENT_SHADER, gl.MEDIUM_INT);
    // var _fragmentShaderPrecisionLowpInt =
    //     _gl.getShaderPrecisionFormat(gl.FRAGMENT_SHADER, gl.LOW_INT);
  ...
  deallocateGeometry(Geometry geometry) {
    ...
    if (geometry.geometryGroups != null) {
      for (var g in geometry.geometryGroups.keys) {
  ...
  initMeshBuffers(WebGLGeometry geometryGroup, Object3D object) {
    ...
    if (!object.geometry.skinWeights.isEmpty &&
        !object.geometry.skinIndices.isEmpty) {
  ...
  Material getBufferMaterial(Object3D object, WebGLGeometry geometry) {
    return (object.material is MeshFaceMaterial)
        ? (object.material as MeshFaceMaterial)
  ...
  int bufferGuessNormalType(Material material) {
    ...
    if (((material is MeshBasicMaterial) && (material.envMap == null)) ||
        (material is MeshDepthMaterial)) {
      return NoShading;
    }
  ...
  bool bufferGuessUVType(Material material) {
    ...
    if (((material is TextureMapping) &&
        (material as TextureMapping).map != null)) {
      return true;
    }
  ...
  setParticleBuffers(Geometry geometry, int hint, object) {
    ...
    if (object.sortParticles) {
      ...
      var sortArray = enumerate(vertices.map((v) {
        v = v.clone()..applyProjection(_projScreenMatrixPS);
        return v.z;
      })).toList();

      sortArray.sort(indexedValueCompare);

      for (v = 0; v < vl; v++) {
        vertex = vertices[sortArray[v].index];
      ...
      for (c = 0; c < cl; c++) {
        ...
        color = colors[sortArray[c].index];
      ...
      if (customAttributes != null) {
        ...
        for (i = 0; i < il; i++) {
          ...
          if (!(customAttribute.boundTo == null ||
              customAttribute.boundTo == "vertices")) continue;
  ...
  setMeshBuffers(WebGLGeometry geometryGroup, Object3D object, int hint,
      bool dispose, Material material) {
    ...
    var f,
        ...
        obj_uvs = (geometry.faceVertexUvs.length == 0)
            ? []
            : geometry.faceVertexUvs[0],
        obj_uvs2 = (geometry.faceVertexUvs.length > 1)
    ...
    if (dirtyMorphTargets) {
      ...
      for (vk = 0; vk < vkl; vk++) {
        ...
        _gl.bindBuffer(
            gl.ARRAY_BUFFER, geometryGroup.__webglMorphTargetsBuffers[vk]);
        _gl.bufferData(gl.ARRAY_BUFFER, morphTargetsArrays[vk], hint);

        if ((material is Morphing) && (material as Morphing).morphNormals) {
          _gl.bindBuffer(
              gl.ARRAY_BUFFER, geometryGroup.__webglMorphNormalsBuffers[vk]);
    ...
    if (dispose) {
      geometryGroup.__inittedArrays =
          false; //delete geometryGroup.__inittedArrays"];
      geometryGroup.__colorArray = null; //delete geometryGroup.__colorArray"];
      geometryGroup.__normalArray =
          null; //delete geometryGroup.__normalArray"];
      geometryGroup.__tangentArray =
          null; //delete geometryGroup.__tangentArray"];
      ...
      geometryGroup.__vertexArray =
          null; //delete geometryGroup.__vertexArray"];
      geometryGroup.__lineArray = null; //delete geometryGroup.__lineArray"];
      geometryGroup.__skinIndexArray =
          null; //delete geometryGroup.__skinIndexArray"];
      geometryGroup.__skinWeightArray =
          null; //delete geometryGroup.__skinWeightArray"];
  ...
  renderBufferDirect(Camera camera, List<Light> lights, fog, Material material,
      BufferGeometry geometry, Object object) {
    ...
    var updateBuffers = false,
        wireframeBit =
            ((material is Wireframe) && (material as Wireframe).wireframe)
                ? 1
                : 0,
        geometryHash =
            (geometry.id * 0xffffff) + (program.id * 2) + wireframeBit;
    ...
    if (object is Mesh) {
      ...
      if (index != null) {
        ...
        for (var i = 0, il = offsets.length; i < il; i++) {
          ...
          if (updateBuffers) {
            ...
            if (attributes["normal"] >= 0 && normal != null) {
              ...
              _gl.vertexAttribPointer(attributes["normal"], normalSize,
                  gl.FLOAT, false, 0, startIndex * normalSize * 4);
            }
            ...
            if (attributes["uv"] >= 0 && uv != null) {
              ...
              _gl.vertexAttribPointer(attributes["uv"], uvSize, gl.FLOAT, false,
                  0, startIndex * uvSize * 4);
            }
            ...
            if (attributes["color"] >= 0 && color != null) {
              ...
              _gl.vertexAttribPointer(attributes["color"], colorSize, gl.FLOAT,
                  false, 0, startIndex * colorSize * 4);
            }
            ...
            if (attributes["tangent"] >= 0 && tangent != null) {
              ...
              _gl.vertexAttribPointer(attributes["tangent"], tangentSize,
                  gl.FLOAT, false, 0, startIndex * tangentSize * 4);
            }
          ...
          _gl.drawElements(gl.TRIANGLES, offsets[i].count, gl.UNSIGNED_SHORT,
        ...
        if (updateBuffers) {
          ...
          enableAttribute(attributes["position"]);
          _gl.vertexAttribPointer(
              attributes["position"], positionSize, gl.FLOAT, false, 0, 0);
          ...
          if (attributes["normal"] >= 0 && normal != null) {
            ...
            enableAttribute(attributes["normal"]);
            _gl.vertexAttribPointer(
                attributes["normal"], normalSize, gl.FLOAT, false, 0, 0);
          }
          ...
          if (attributes["uv"] >= 0 && uv != null) {
            ...
            enableAttribute(attributes["uv"]);
            _gl.vertexAttribPointer(
                attributes["uv"], uvSize, gl.FLOAT, false, 0, 0);
          }
          ...
          if (attributes["color"] >= 0 && color != null) {
            ...
            enableAttribute(attributes["color"]);
            _gl.vertexAttribPointer(
                attributes["color"], colorSize, gl.FLOAT, false, 0, 0);
          }
          ...
          if (attributes["tangent"] >= 0 && tangent != null) {
            ...
            enableAttribute(attributes["tangent"]);
            _gl.vertexAttribPointer(
                attributes["tangent"], tangentSize, gl.FLOAT, false, 0, 0);
          }
        }
  ...
  renderBuffer(Camera camera, List<Light> lights, Fog fog, Material material,
      WebGLGeometry geometryGroup, Object3D object) {
    if (!material.visible) return;
    ...
    if (((material is! Morphing) ||
            ((material is Morphing) && !(material as Morphing).morphTargets)) &&
        attributes["position"] >= 0) {
      if (updateBuffers) {
        _gl.bindBuffer(gl.ARRAY_BUFFER, geometryGroup.__webglVertexBuffer);
        enableAttribute(attributes["position"]);
        _gl.vertexAttribPointer(
            attributes["position"], 3, gl.FLOAT, false, 0, 0);
      }
    } else {
    ...
    if (updateBuffers) {
      ...
      if (attributes["tangent"] >= 0) {
        ...
        enableAttribute(attributes["tangent"]);
        _gl.vertexAttribPointer(
            attributes["tangent"], 4, gl.FLOAT, false, 0, 0);
      }
      ...
      if ((material is Skinning) &&
        ...
        _gl.vertexAttribPointer(
            attributes["skinIndex"], 4, gl.FLOAT, false, 0, 0);

        _gl.bindBuffer(gl.ARRAY_BUFFER, geometryGroup.__webglSkinWeightsBuffer);
        enableAttribute(attributes["skinWeight"]);
        _gl.vertexAttribPointer(
            attributes["skinWeight"], 4, gl.FLOAT, false, 0, 0);
      }
      ...
      if (attributes["lineDistance"] >= 0) {
        _gl.bindBuffer(
            gl.ARRAY_BUFFER, geometryGroup.__webglLineDistanceBuffer);
        enableAttribute(attributes["lineDistance"]);
        _gl.vertexAttribPointer(
            attributes["lineDistance"], 1, gl.FLOAT, false, 0, 0);
      }
    }
    ...
    if (object is Mesh) {
      ...
      if (material is Wireframe && (material as Wireframe).wireframe) {
        ...
        if (updateBuffers)
          _gl.bindBuffer(
              gl.ELEMENT_ARRAY_BUFFER, geometryGroup.__webglLineBuffer);
        _gl.drawElements(
            gl.LINES, geometryGroup.__webglLineCount, gl.UNSIGNED_SHORT, 0);
        ...
        if (updateBuffers)
          _gl.bindBuffer(
              gl.ELEMENT_ARRAY_BUFFER, geometryGroup.__webglFaceBuffer);
        _gl.drawElements(
            gl.TRIANGLES, geometryGroup.__webglFaceCount, gl.UNSIGNED_SHORT, 0);
      }
  ...
  setupMorphTargets(
      Material material, WebGLGeometry geometryGroup, Mesh object) {
    // set base

    var attributes = material._program.attributes;

    if (object.morphTargetBase != -1 && attributes["position"] >= 0) {
      _gl.bindBuffer(gl.ARRAY_BUFFER,
    ...
    if ((material is Morphing) && object.morphTargetForcedOrder.length > 0) {
      ...
      while (m < (material as Morphing).numSupportedMorphTargets &&
        ...
        if (attributes["morphTarget$m"] >= 0) {
          ...
          _gl.vertexAttribPointer(
              attributes["morphTarget$m"], 3, gl.FLOAT, false, 0, 0);
        }

        if (attributes["morphNormal$m"] >= 0 &&
            (material as Morphing).morphNormals) {
          _gl.bindBuffer(gl.ARRAY_BUFFER,
              geometryGroup.__webglMorphNormalsBuffers[order[m]]);
          enableAttribute(attributes["morphNormal$m"]);
          _gl.vertexAttribPointer(
              attributes["morphNormal$m"], 3, gl.FLOAT, false, 0, 0);
        }

        object.__webglMorphTargetInfluences[m] =
            influences[order[m]].toDouble();
      ...
      if (indexedActiveInfluences.length >
        ...
        indexedActiveInfluences.sort(indexedValueCompare);
        indexedActiveInfluences.length =
            (material as Morphing).numSupportedMorphTargets;
      } else if (indexedActiveInfluences.length >
          (material as Morphing).numSupportedMorphNormals) {
        indexedActiveInfluences.sort(indexedValueCompare);
      } else if (indexedActiveInfluences.length == 0) {
        indexedActiveInfluences.add(new IndexedValue<int>(0, 0));
      }
      ...
      if (material is Morphing) {
        while (m < (material as Morphing).numSupportedMorphTargets) {
          if (m < indexedActiveInfluences.length &&
              indexedActiveInfluences[m] != null) {
            influenceIndex = indexedActiveInfluences[m].index;
            ...
            object.__webglMorphTargetInfluences[m] =
                influences[influenceIndex].toDouble();
          } else {
  ...
  painterSort(a, b) =>
      (a.z.isNaN || b.z.isNaN || a.z.isInfinite || b.z.isInfinite)
          ? 0
          : (b.z - a.z).toInt();
  ...
  void renderToTarget(
      Scene scene, Camera camera, WebGLRenderTarget renderTarget,
      [forceClear = false]) {
  ...
  _render(Scene scene, Camera camera, {renderTarget: null, forceClear: false}) {
    ...
    if (scene.overrideMaterial != null) {
      ...
      setBlending(material.blending, material.blendEquation, material.blendSrc,
          material.blendDst);
      ...
      renderObjects(
          scene.__webglObjects, false, "", camera, lights, fog, true, material);
      renderObjectsImmediate(scene.__webglObjectsImmediate, "", camera, lights,
          fog, false, material);
    } else {
      var material = null;
      ...
      renderObjects(scene.__webglObjects, true, "opaque", camera, lights, fog,
          false, material);
      renderObjectsImmediate(scene.__webglObjectsImmediate, "opaque", camera,
          lights, fog, false, material);

      // transparent pass (back-to-front order)

      renderObjects(scene.__webglObjects, false, "transparent", camera, lights,
          fog, true, material);
      renderObjectsImmediate(scene.__webglObjectsImmediate, "transparent",
  ...
  renderObjects(List<WebGLObject> renderList, bool reverse, String materialType,
      Camera camera, List<Light> lights, fog, useBlending,
  ...
  renderObjectsImmediate(
      renderList, materialType, camera, lights, fog, useBlending,
      [overrideMaterial]) {
    var webglObject, object, material, il;
  ...
  renderImmediateObject(
      Camera camera, List<Light> lights, Fog fog, Material material, object) {
  ...
  unrollImmediateBufferMaterial(WebGLObject webglobject) {
    var object = webglobject.object, material = object.material;
  ...
  sortFacesByMaterial(Geometry geometry, Material material) {
    var f, fl, face, materialIndex, vertices, groupHash;
    ...
    if (!object.__webglInit) {
      ...
      if (object is Mesh) {
        ...
        if ((object.geometry is Geometry) &&
            (object.geometry is! BufferGeometry)) {
          if (geometry.geometryGroups == null) {
      ...
      if (object.geometry is BufferGeometry) {
        ...
        for (var i = 0, il = geometry.geometryGroupsList.length; i < il; i++) {
          ...
          customAttributesDirty =
              hasAttributes && areCustomAttributesDirty(material);
          ...
          if (geometry.verticesNeedUpdate ||
              ...
              geometry.tangentsNeedUpdate ||
              customAttributesDirty) {
            setMeshBuffers(geometryGroup, object, gl.DYNAMIC_DRAW,
                !geometry.isDynamic, material);
          }
        }
  ...
  areCustomAttributesDirty(material) =>
      material.attributes.values.any((a) => a.needsUpdate);
  ...
  initMaterial(
    ...
    material._program = buildProgram(
        ...
        map: (material is TextureMapping)
            ? (material as TextureMapping).map
            : null,
        envMap: (material is EnvironmentMapping)
            ? (material as EnvironmentMapping).envMap
            : null,
        lightMap: (material is EnvironmentMapping)
            ? (material as EnvironmentMapping).lightMap
            : null,
        bumpMap: (material is BumpMapping)
            ? (material as BumpMapping).bumpMap
            : null,
        normalMap: (material is BumpMapping)
            ? (material as BumpMapping).normalMap
            : null,
        specularMap: (material is EnvironmentMapping)
            ? (material as EnvironmentMapping).specularMap
            : null,
        vertexColors: material.vertexColors,
        fog: fog,
        useFog: material.fog,
        fogExp: fog is FogExp2,
        sizeAttenuation: (material is ParticleBasicMaterial)
            ? material.sizeAttenuation
            : false,
        skinning:
            (material is Skinning) ? (material as Skinning).skinning : false,
        maxBones: maxBones,
        useVertexTexture: supportsBoneTextures &&
            object != null &&
            object is SkinnedMesh &&
            object.useVertexTexture,
        boneTextureWidth: (object != null && object is SkinnedMesh)
            ? object.boneTextureWidth
            : 0,
        boneTextureHeight: (object != null && object is SkinnedMesh)
            ? object.boneTextureHeight
            : 0,
        morphTargets: (material is Morphing)
            ? (material as Morphing).morphTargets
            : false,
  ...
  setProgram(Camera camera, List<Light> lights, Fog fog, Material material,
      Object3D object) {
    _usedTextureUnits = 0;
    ...
    if ((material is Morphing) && (material as Morphing).morphTargets) {
      if ((object as Mesh).__webglMorphTargetInfluences == null) {
        (object as Mesh).__webglMorphTargetInfluences =
            new Float32List(maxMorphTargets);
      }
    }
      ...
      if (supportsBoneTextures && object.useVertexTexture) {
        ...
        if (p_uniforms.boneGlobalMatrices != null) {
          _gl.uniformMatrix4fv(
              p_uniforms.boneGlobalMatrices, false, object.boneMatrices);
        }
      }
    }
    ...
    if (refreshMaterial) {
      ...
      if ((material is ShaderMaterial) ||
          (material is MeshPhongMaterial) ||
          ((material is EnvironmentMapping) &&
              (material as EnvironmentMapping).envMap != null)) {
  ...
  refreshUniformsCommon(Map<String, Uniform> uniforms, Material material) {
    ...
    if (material is EnvironmentMapping) {
      ...
      uniforms["specularMap"].value =
          (material as EnvironmentMapping).specularMap;
    }

    if ((material is BumpMapping) &&
        (material as BumpMapping).bumpMap != null) {
    ...
    } else if ((material is EnvironmentMapping) &&
        (material as EnvironmentMapping).specularMap != null) {
      uvScaleMap = (material as EnvironmentMapping).specularMap;
    } else if ((material is BumpMapping) &&
        (material as BumpMapping).normalMap != null) {
    ...
    if (uvScaleMap != null) {
      ...
      uniforms["offsetRepeat"]
          .value
          .setValues(offset.x, offset.y, repeat.x, repeat.y);
    }
    ...
    if (material is EnvironmentMapping) {
      ...
      uniforms["flipEnvMap"].value =
          (envMat.envMap is WebGLRenderTargetCube) ? 1 : -1;
      ...
      uniforms["useRefract"].value = ((envMat.envMap != null) &&
              (envMat.envMap.mapping is CubeRefractionMapping))
          ? 1
          : 0;
    }
  }
  ...
  refreshUniformsLine(
      Map<String, Uniform> uniforms, LineBasicMaterial material) {
    uniforms["diffuse"].value = material.color;
    uniforms["opacity"].value = material.opacity;
  }
  ...
  int getTextureUnit() {
    ...
    if (unit >= maxTextures) {
      print(
          "WebGLRenderer: trying to use $unit texture units while this GPU supports only $maxTextures");
    }
  ...
  setColorGamma(
      List<double> array, int offset, Color color, double intensitySq) {
  ...
  setColorLinear(
      List<double> array, int offset, Color color, double intensity) {
  ...
  Program buildProgram(String shaderID, String fragmentShader,
      String vertexShader, Map uniforms, Map attributes, Map defines,
      {int maxDirLights: 0,
    ...
    var prefix_vertex = [
      ...
      (((vertexColors is bool) && vertexColors) ||
              ((vertexColors is int) && (vertexColors != NoColors)))
          ? "#define USE_COLOR"
          : "",
      skinning ? "#define USE_SKINNING" : "",
      useVertexTexture ? "#define BONE_TEXTURE" : "",
      (boneTextureWidth != null)
          ? "#define N_BONE_PIXEL_X ${boneTextureWidth.toStringAsFixed(1)}"
          : "",
      (boneTextureHeight != null)
          ? "#define N_BONE_PIXEL_Y ${boneTextureHeight.toStringAsFixed( 1 )}"
          : "",
    ...
    var glFragmentShader =
        getShader("fragment", "$prefix_fragment$fragmentShader");
    ...
    var program =
        new Program(_programs_counter++, glprogram, code, usedTimes: 1);
  ...
  setTexture(Texture texture, int slot) {
    if (texture.needsUpdate) {
      ...
      _gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL,
      ...
      if (texture is DataTexture) {
        ...
        if (mipmaps.length > 0 && isImagePowerOfTwo) {
          for (var i = 0, il = mipmaps.length; i < il; i++) {
            mipmap = mipmaps[i];
            _gl.texImage2D(gl.TEXTURE_2D, i, glFormat, mipmap.width,
        ...
        for (var i = 0, il = mipmaps.length; i < il; i++) {
          mipmap = mipmaps[i];
          _gl.compressedTexImage2D(gl.TEXTURE_2D, i, glFormat, mipmap.width,
  ...
  clampToMaxSize(image, num maxSize) {
    ...
    var maxDimension = Math.max<num>(image.width, image.height);
    ...
    ctx.drawImageScaledFromSource(
        image, 0, 0, image.width, image.height, 0, 0, newWidth, newHeight);
        ...
        for (var i = 0; i < 6; i++) {
          if (isCompressed) {
            ...
            for (var j = 0, jl = mipmaps.length; j < jl; j++) {
              mipmap = mipmaps[j];
              _gl.compressedTexImage2D(gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, j,
                  glFormat, mipmap.width, mipmap.height, 0, mipmap.data);
            }
          } else {
            // _gl.texImage2DImage(
            _gl.texImage2D(
  ...
  setRenderTarget(WebGLRenderTarget renderTarget) {
    if ((renderTarget != null) && (renderTarget.__webglFramebuffer == null)) {
      ...
      if (renderTarget is WebGLRenderTargetCube) {
        ...
        _gl.texImage2D(gl.TEXTURE_2D, 0, glFormat, renderTarget.width,
...
class WebGLRendererMemoryInfo {
  int programs = 0, geometries = 0, textures = 0;
}

class WebGLRendererRenderInfo {
  int calls = 0, vertices = 0, faces = 0, points = 0;
}
...
class WebGLObject {
  ...
  WebGLObject(this.object, this.opaque, this.transparent, this.buffer,
